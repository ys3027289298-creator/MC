...
export class GameEngine {
  ...
  continueSave() {
    ...
    const d = result.data;
    // Build the full restored state first, then swap it in atomically, so a
    // failure here can never half-replace the live world/body/inventory/etc.
    let restored: {
      world: World;
      body: PlayerBody;
      inventory: Inventory;
    ...
    try {
      ...
      showError(
        this.uiRoot,
        `存档已损坏，读取失败：${e instanceof Error ? e.message : String(e)}`,
        () => this.showMenu()
      );
      return;
    }
